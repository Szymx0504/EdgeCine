...
for u_id in range(1, 201):
    ...
    # Boost interactions: 30-60 items per user instead of 5-15
    final_selection = random.sample(target_films, k=min(len(target_films), random.randint(30, 60)))

    for f_id in final_selection:
        # Favoring higher ratings (4-5)
        # Using weights: 1:5%, 2:5%, 3:15%, 4:45%, 5:30%
        rating = random.choices([1, 2, 3, 4, 5], weights=[5, 5, 15, 45, 30])[0]
        act_type = 'like' if random.random() < 0.4 else f'rate_{rating}'
...
# Ensure EVERY film has engagement (20-50 random interactions)
for film_id in range(1, len(df) + 1):
    for _ in range(random.randint(20, 50)):
        u_id = random.randint(1, 200)
        # Shift ratings even higher for background noise (4-5 mostly)
        rating = random.choices([3, 4, 5], weights=[10, 50, 40])[0]
        act_type = 'like' if random.random() < 0.25 else f'rate_{rating}'
        ts = (datetime.now() - timedelta(days=random.randint(1, 730))).strftime('%Y-%m-%d %H:%M:%S')
        user_interactions_items.append(f"({u_id}, {film_id}, '{act_type}', '{ts}')")
...
with open(sql_path, "w", encoding="utf-8") as out:
    ...
    # Clean existing data to allow re-seeding without volume reset
    out.write("TRUNCATE films, users, actors, tags, user_interactions, movies, series CASCADE;\n\n")
